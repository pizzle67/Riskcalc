...
let seHistogramChart = null;
let seExceedanceChart = null;
...
// Switch between Annual and Single Event results tabs
function switchResultsTab(tabName) {
    document.querySelectorAll('.results-tab').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.tab === tabName);
    });
    document.querySelectorAll('.results-tab-content').forEach(panel => {
    ...
    // Charts may need a resize after becoming visible
    if (tabName === 'single-event') {
        if (seHistogramChart) seHistogramChart.resize();
        if (seExceedanceChart) seExceedanceChart.resize();
    } else {
        if (histogramChart) histogramChart.resize();
        if (exceedanceChart) exceedanceChart.resize();
        if (likelihoodChart) likelihoodChart.resize();
    }
}
...
function displayResults(data, analysisMode) {
    ...
    // Tabs: show for ALE (which has both views), hide for SLE
    const tabBar = document.getElementById('results-tabs');
    const singleEventTab = document.getElementById('tab-single-event');
    if (isSLE) {
        tabBar.style.display = 'none';
        singleEventTab.style.display = 'none';
        document.getElementById('tab-annual').style.display = 'block';
    } else {
        tabBar.style.display = 'flex';
        switchResultsTab('annual');
    }
    ...
    // Likelihood / event count chart (ALE mode only)
    const likelihoodSection = document.getElementById('likelihood-section');
    if (!isSLE && data.likelihood) {
        likelihoodSection.style.display = 'block';
        document.getElementById('zero-event-pct').textContent =
            data.likelihood.zero_event_pct.toFixed(1) + '%';
        updateLikelihoodChart(data.likelihood);
    } else {
        likelihoodSection.style.display = 'none';
    }

    // Single Event view (ALE mode only)
    if (!isSLE && data.single_event) {
        populateSingleEventView(data);
    }
...
// Populate the Single Event results tab
function populateSingleEventView(data) {
    const se = data.single_event;

    // Probability cards
    document.getElementById('se-event-prob').textContent = se.event_probability.toFixed(1) + '%';
    document.getElementById('se-mean').textContent = formatCurrency(se.summary.mean);
    document.getElementById('se-var').textContent = formatCurrency(se.summary.var_95);
    document.getElementById('se-cvar').textContent = formatCurrency(se.summary.cvar_95);

    // Percentiles table
    document.getElementById('se-p10').textContent = formatCurrency(se.summary.p10);
    document.getElementById('se-p25').textContent = formatCurrency(se.summary.p25);
    document.getElementById('se-median').textContent = formatCurrency(se.summary.median);
    document.getElementById('se-p75').textContent = formatCurrency(se.summary.p75);
    document.getElementById('se-p90').textContent = formatCurrency(se.summary.p90);
    document.getElementById('se-p95').textContent = formatCurrency(se.summary.p95);

    // Event statistics table
    document.getElementById('se-prob-detail').textContent = se.event_probability.toFixed(1) + '%';
    document.getElementById('se-no-event').textContent = data.likelihood.zero_event_pct.toFixed(1) + '%';
    document.getElementById('se-lef').textContent = formatNumber(data.lef.mean) + ' events/year';
    document.getElementById('se-min').textContent = formatCurrency(se.summary.min);
    document.getElementById('se-max').textContent = formatCurrency(se.summary.max);
    document.getElementById('se-std').textContent = formatCurrency(se.summary.std);

    // Single event charts
    updateSEHistogramChart(se.histogram);
    updateSEExceedanceChart(se.exceedance);
}
...
// Create/update single-event histogram chart
function updateSEHistogramChart(histogramData) {
    const ctx = document.getElementById('se-histogram-chart').getContext('2d');
    ...
    if (seHistogramChart) {
        seHistogramChart.destroy();
    }

    seHistogramChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: binCenters.map(v => formatCurrency(v)),
            datasets: [{
        ...
        options: {
            ...
            scales: {
                x: {
                    display: true,
                    title: { display: true, text: 'Single Event Loss ($)' },
...
// Create/update single-event exceedance curve chart
function updateSEExceedanceChart(exceedanceData) {
    const ctx = document.getElementById('se-exceedance-chart').getContext('2d');

    if (seExceedanceChart) {
        seExceedanceChart.destroy();
    }

    seExceedanceChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: exceedanceData.values.map(v => formatCurrency(v)),
            datasets: [{
...
// Create/update likelihood (event count) chart
function updateLikelihoodChart(likelihoodData) {
    ...
    const labels = likelihoodData.event_labels.map(n =>
        n === 0 ? '0 (No Loss)' : String(n)
    );

    const barColors = likelihoodData.event_labels.map(n =>
        n === 0 ? 'rgba(16, 185, 129, 0.7)' : 'rgba(37, 99, 235, 0.6)'
    );
    const borderColors = likelihoodData.event_labels.map(n =>
        n === 0 ? 'rgba(16, 185, 129, 1)' : 'rgba(37, 99, 235, 1)'
    );
    ...
    likelihoodChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Probability (%)',
                data: likelihoodData.probabilities,
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            return context.parsed.y.toFixed(1) + '% of iterations';
                        }
                    }
                }
            },
...
async function runSimulation(event) {
    ...
    try {
        ...
        const endpoint = analysisMode === 'sle' ? `${BASE_PATH}/api/simulate-sle` : `${BASE_PATH}/api/simulate`;
...
async function loadScenarios() {
    try {
        const response = await fetch(`${BASE_PATH}/api/scenarios`);
        const data = await response.json();
...
async function loadScenario(scenarioId) {
    ...
    try {
        const response = await fetch(`${BASE_PATH}/api/scenarios/${scenarioId}`);
        const data = await response.json();
...
async function saveScenario(saveAsNew = false) {
    ...
    try {
        ...
        if (currentScenarioId && !saveAsNew) {
            ...
            response = await fetch(`${BASE_PATH}/api/scenarios/${currentScenarioId}`, {
                method: 'PUT',
            ...
            // Create new scenario
            response = await fetch(`${BASE_PATH}/api/scenarios`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(scenarioData)
            });
...
async function deleteScenario() {
    ...
    try {
        const response = await fetch(`${BASE_PATH}/api/scenarios/${currentScenarioId}`, {
            method: 'DELETE'
        });
...
async function loadHistory() {
    ...
    try {
        const response = await fetch(`${BASE_PATH}/api/scenarios/${currentScenarioId}/results`);
        const data = await response.json();
...
async function viewResult(resultId) {
    try {
        const response = await fetch(`${BASE_PATH}/api/results/${resultId}`);
        const data = await response.json();
